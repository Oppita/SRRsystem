        ...
        {filteredComisiones.map(comision => {
          ...
          return (
            <motion.div 
              ...
              className="bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all cursor-pointer overflow-hidden" 
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-black text-lg text-slate-900 leading-tight">
                        {comision.municipios || 'Sin destino'}
                      </h3>
                      <p className="text-slate-500 text-xs font-medium mt-1 uppercase tracking-wider">{comision.tipoComision || 'Comisión Técnica'}</p>
                    </div>
                  </div>
                  
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1 ${
                    comision.estado === 'Ejecutada' ? 'bg-emerald-50 text-emerald-700' :
                    comision.estado === 'En Curso' ? 'bg-blue-50 text-blue-700' :
                    comision.estado === 'Programada' ? 'bg-amber-50 text-amber-700' :
                    'bg-rose-50 text-rose-700'
                  }`}>
                    {comision.estado === 'Ejecutada' ? <CheckCircle2 size={12} /> :
                     comision.estado === 'En Curso' ? <Clock size={12} /> :
                     comision.estado === 'Programada' ? <Calendar size={12} /> :
                     <AlertTriangle size={12} />}
                    {comision.estado}
                  </div>
                </div>

                <p className="text-slate-600 text-sm mb-6 leading-relaxed bg-slate-50 p-4 rounded-2xl border border-slate-100 italic">
                  "{comision.objeto}"
                </p>

                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Fechas</p>
                    <p className="text-xs font-bold text-slate-800">{comision.fechaInicio} / {comision.fechaFin}</p>
                  </div>
                  <div className="bg-indigo-50 p-3 rounded-xl border border-indigo-100">
                    <p className="text-[10px] font-bold text-indigo-400 uppercase mb-1">Costo Total</p>
                    <p className="text-xs font-black text-indigo-900">${comision.costoTotal.toLocaleString('es-CO')}</p>
                  </div>
                </div>
