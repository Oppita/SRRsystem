...
import { MapPin, Users, Briefcase, Award, TrendingUp, Target, X } from 'lucide-react';
      ...
      <div className="flex flex-col gap-6 h-[600px] overflow-y-auto pr-2 custom-scrollbar">
        {selectedDept ? (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 animate-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-4">
              <h4 className="font-black text-slate-400 text-[10px] uppercase tracking-widest border-b border-slate-100 pb-2">Contratistas en el Territorio</h4>
              {departmentStats[selectedDept]?.contractors.map(cont => {
                ...
                return (
                  <div key={cont.id} className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden">
                    <button 
                    ...
                    >
                      <div>
                        <p className="font-bold text-slate-800 group-hover:text-blue-700 transition-colors">{cont.nombre}</p>
                        <p className="text-[10px] text-slate-500 font-medium">NIT: {cont.nit}</p>
                      </div>
                      <div className="bg-white px-2 py-1 rounded-lg border border-slate-200 text-[10px] font-black text-blue-600">
                        {contProjects.length} PROYECTOS
                      </div>
                    </button>
                    
                    <div className="px-4 pb-4 space-y-2">
                      {contProjects.map(proj => (
                        <button
                          key={proj.id}
                          onClick={() => onSelectProject(proj.id)}
                          className="w-full p-3 bg-white rounded-xl border border-slate-100 text-left hover:border-blue-300 hover:shadow-sm transition-all flex items-center gap-3 group/proj"
                        >
                          <div className="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 flex items-center justify-center group-hover/proj:bg-blue-50 group-hover/proj:text-blue-500 transition-colors">
                            <Briefcase size={14} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-slate-700 truncate">{proj.nombre}</p>
                            <p className="text-[10px] text-slate-500">{proj.municipio}</p>
                          </div>
                        </button>
                      ))}
                    </div>
