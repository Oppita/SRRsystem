...
export const SurveyModule: React.FC<{ onExit?: () => void }> = ({ onExit }) => {
  const { state, addSurvey, addSurveyResponse, addSurveyAnalysis, deleteSurvey } = useProject();
...
const SurveyTaker: React.FC<{ 
  ...
  const handleSubmit = () => {
    ...
    if (!surveyor.fullName || !respondent.fullName) {
      showAlert('La información del encuestador y respondiente es obligatoria para la validez legal.');
      return;
    }
  ...
  const updateAnswer = (qid: string, val: any) => {
    ...
    // Update progress
    const answeredCount = Object.keys(newAnswers).filter(k => newAnswers[k]).length;
  ...
  const performAIAnalysis = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        RESULTADOS (${responses.length} respuestas recolectadas):
        ${responses.map((r, i) => `
        REGISTRO #${i+1}:
        - Geolocalización: ${r.departamentoId} -> ${r.municipioId}
        ...
        - Auditoría: Encuestador: ${r.surveyorInfo?.fullName} | Ciudadano: ${r.respondentInfo?.fullName} (Edad: ${r.respondentInfo?.age})
        - Datos Capturados: ${JSON.stringify(r.answers)}
        `).join('\n')}
        
        POR FAVOR GENERA UN INFORME DE DECISIÓN CRÍTICO (Markdown):
        1. Estado de "Pobreza Expuesta al Riesgo": Cruza los datos de ingresos/comida con la exposición a inundaciones.
        2. Caracterización Socioeconómica y Medios de Vida.
        3. Identificación de "Invisibilidad Estadística": Zonas donde la pobreza se cruza con falta de servicios.
        4. Alertas de Reasentamiento o Adaptación basada en Comunidades.
        5. Medición de Capacidad Adaptativa vs Fragilidad Estructural.
        
        IMPORTANTE: Define el indicador de "Pobreza Expuesta al Riesgo" como el eje central del análisis.
        
        Al final, proporciona 5 indicadores clave (0-100) en formato JSON:
        { "indicators": [
            { "label": "Pobreza Expuesta", "value": X, "color": "red" },
            { "label": "Vulnerabilidad Social", "value": X, "color": "yellow" },
            { "label": "Fragilidad Territorial", "value": X, "color": "red" },
            { "label": "Capacidad Adaptativa", "value": X, "color": "emerald" },
            { "label": "Riesgo Multidimensional", "value": X, "color": "yellow" }
          ]
        }
      `;

      const text = await aiProviderService.generateContent(prompt, "gemini-3-flash-preview", { responseMimeType: 'application/json' });
