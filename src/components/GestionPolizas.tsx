...
import { ShieldCheck, Plus, Search, Filter, Calendar, FileText, AlertCircle, CheckCircle2, Clock, XCircle, Trash2, Edit2, ExternalLink, ArrowRight, TrendingUp, DollarSign, Percent, Loader2, Upload, BrainCircuit, AlertTriangle, FileWarning, Link2, User, Scale, Shield, Zap, X, Bell } from 'lucide-react';
...
export const GestionPolizas: React.FC<GestionPolizasProps> = ({ projectId }) => {
  ...
  const [viewMode, setViewMode] = useState<'table' | 'cards' | 'timeline' | 'reports'>('cards');
  ...
  const filteredPolizas = useMemo(() => {
    ...
    return polizas.filter(p => {
      const matchesSearch = 
        (p.numero_poliza?.toString() || '').toLowerCase().includes((searchTerm || '').toLowerCase()) ||
        (p.entidad_aseguradora || '').toLowerCase().includes((searchTerm || '').toLowerCase()) ||
        (p.numero_contrato?.toString() || '').toLowerCase().includes((searchTerm || '').toLowerCase());
      
      const matchesEstado = filterEstado === 'all' || p.estado === filterEstado;
      
      return matchesSearch && matchesEstado;
    });
  }, [state.polizas, projectId, searchTerm, filterEstado]);
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex flex-1 gap-4 w-full md:w-auto">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por póliza, aseguradora o contrato..."
              value={searchTerm}
