...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const recalculateAll = useCallback((inputState: GlobalState): GlobalState => {
    try {
      ...
      if (currentState.proyectos.length > 0) {
        console.log(`Recalculando ${currentState.proyectos.length} proyectos...`);
      }

    // 1. Update Contracts
    const updatedContratos = currentState.contratos.map(contract => {
      const totals = calculateContractTotals(contract, currentState.otrosies);
      return {
        ...contract,
    ...
    const updatedProyectos = currentState.proyectos.map(project => {
      ...
      // Calculate physical progress based on latest reports or advances
      const projectReports = currentState.informesInterventoria.filter(r => r.projectId === project.id);
      const projectAvances = currentState.avances.filter(a => a.projectId === project.id);
    ...
    // 6. Financial Traceability and Audit
    ...
    cdps.forEach(cdp => {
      ...
      const valorPagado = relatedDocs.reduce((sum, r) => sum + (r.valorPagado || 0), 0);
  ...
  useEffect(() => {
    if (!loading) {
      try {
        // Optimization: Exclude static data (municipios, departamentos) to save space
        // These are re-populated from EMPTY_STATE on load
        const { municipios, departamentos, ...stateToSave } = state;
        localStorage.setItem(STORAGE_KEY, JSON.stringify(stateToSave));
      } catch (err: any) {
        // Handle QuotaExceededError gracefully
        if (err.name === 'QuotaExceededError' || err.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
          console.warn('LocalStorage quota exceeded. Attempting to save a thinner state...');
          try {
            // Try to save without heavy analysis fields which are the main space consumers
            const thinnerState = {
              ...state,
              municipios: [],
              departamentos: [],
              documentos: state.documentos.map(d => ({ ...d, analysis: undefined })),
              contratos: state.contratos.map(c => ({ ...c, analysis: undefined }))
            };
            localStorage.setItem(STORAGE_KEY, JSON.stringify(thinnerState));
          } catch (innerErr) {
            console.error('Failed to save even thinner state to LocalStorage:', innerErr);
            // If it still fails, we rely on Supabase which is already being saved separately
          }
        } else {
  ...
  const addPago = (pago: Pago) => {
    setState(prevState => {
      ...
      const newState = {
        ...
        seguimientos: [...prevState.seguimientos, newSeguimiento]
      };
